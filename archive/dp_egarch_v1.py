"""
dp_egarch_v1.py  --  Adaptive EGARCH v1 (baseline adaptive model)
...
Regularization: L2 on w_omega and w_alpha (lambda=0.01)
No clipping. No smoothness. No EMA.
...
def neg_loglik(params, beta, gamma, eps, X, l2=L2_LAMBDA):
    n_feat = X.shape[1]
    ...
    # L2 penalty
    penalty = l2 * (np.dot(w_omega, w_omega) + np.dot(w_alpha, w_alpha))
...
def main():
    ...
    params_egarch = pd.read_csv('params_egarch.csv')
    ...
    merged = merged.dropna(subset=['rv_yz', 'garch_fc_vol', 'egarch_fc_vol', 'sigma_v1_h1'])
    ...
    rows.append(metrics_row('Adaptive v1',       merged['sigma_v1_h1'].values,  rv, e_g))
    ...
    # ── Stability checks ──────────────────────────────────────────────────────
    print("=" * 72)
    print("STABILITY CHECKS -- Adaptive v1")
    print("=" * 72)
    s = merged['sigma_v1_h1'].values
    ...
    # Save output
    merged[['garch_fc_vol', 'egarch_fc_vol', 'sigma_v1_h1', 'sigma_v1_h5', 'sigma_v1_h21', 'rv_target']].to_csv('data/adaptive_v1_forecasts.csv')
    results.to_csv('data/adaptive_v1_results.csv', index=False)
    print("\nSaved: data/adaptive_v1_forecasts.csv")
    print("Saved: data/adaptive_v1_results.csv")
